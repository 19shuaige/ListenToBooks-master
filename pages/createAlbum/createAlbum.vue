    ...
    <template v-slot:gBody>
      <view class="gui-padding gui-padding-x gui-bg-white">
        <uni-forms label-width="100" ref="formDataRef" :rules="formDataRule" :model="formData">
          ...
          <!--          专辑标签-->
          <uni-forms-item v-if="attributeList.length" label="专辑标签" required name="albumPropertyValueList">
            <!--      标签数据弹出框 -->
            <view class="gui-color-grey2 attr-tags-container" @click="openAttributePopup">
              ...
              <view v-else class="gui-flex gui-wrap gui-m-t-5 gui-m-b-5">
                <uni-tag
                  ...
                  type="primary"
                  v-for="(item,index) in formData.albumPropertyValueList"
                  :key="item.valueId"
                  :text="item.valueName">
                </uni-tag>
              </view>
...
const formData = reactive<AlbumInfoInterface>({
  ...
  vipDiscount: "",
  isOpen: '1',
  albumPropertyValueList: <AlbumAttributeValueVoListInterface[]>[]
})
// 表单校验
const formDataRule = {
  albumTitle: {
...
const selectedAttributeList = computed(() => {
  return formData.albumPropertyValueList.filter(item => item.valueId !== -1)
})
// 付费类型
...
watch(attributeList, (val: AttributeListInterface[]) => {
  formData.albumPropertyValueList = val.map(item => {
    return {
      attributeId: item.id,
...
const getAttributeList = async (categoryId: string | number) => {
  try {
    ...
    res.data.forEach((item: AttributeListInterface, index: number) => {
      ...
      if (formData.albumPropertyValueList?.length){
        // 如果formData中的albumAttributeValueList中有当前属性
        if (formData.albumPropertyValueList.find(item2 => item2.attributeId === item.id)){
          // 选中当前属性
          item.checkedId = formData.albumPropertyValueList.find(item2 => item2.attributeId === item.id)?.valueId || -1
        }else{
            // 没有选中当前属性
          item.checkedId = -1
        }
      } else {
        formData.albumPropertyValueList = []
        item.checkedId = -1
      }
...
const getAlbumDataInfo = async (albumId:number = 2) => {
  try {
    const res = await albumsService.getAlbumInfoById(albumId)
    console.log(res)
    // 付费类型默认值
